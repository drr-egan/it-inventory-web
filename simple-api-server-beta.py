...
class APIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_auth_user_creation(self, data):
        """Handle creation of auth users with password hashing"""
        import hashlib
        ...
        if not all([username, password, firstName, lastName]):
            self.send_error(400, 'Username, password, firstName, and lastName are required')
            return None
        ...
        try:
            cursor.execute('''
                INSERT INTO auth_users (id, username, password, firstName, lastName, role, createdAt)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, username, password_hash, firstName, lastName, role, datetime.now().isoformat()))
            
            conn.commit()
            
            cursor.execute('SELECT id, username, firstName, lastName, role, createdAt FROM auth_users WHERE id = ?', (user_id,))
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif resource == 'auth_users':
                if resource_id:
                    cursor.execute('SELECT id, username, firstName, lastName, role, createdAt, lastLogin FROM auth_users WHERE id = ?', (resource_id,))
                    result = cursor.fetchone()
                    if result is None:
                        self.send_error(404, 'Auth user not found')
                        return
                else:
                    cursor.execute('SELECT id, username, firstName, lastName, role, createdAt, lastLogin FROM auth_users ORDER BY firstName, lastName')
                    result = cursor.fetchall()
    ...
    def do_POST(self):
        ...
        # Handle special auth endpoint
        if resource == 'auth' and len(path_parts) == 2 and path_parts[1] == 'login':
        ...
        try:
            ...
            if resource == 'items':
                ...
                result = self.handle_auth_user_creation(data)
                if result is None:
                    return
                ...
                cursor.execute('''
                    ...
                    (id, item, user, costCode, dateEntered, jobNumber, notes, isUsed, isComplete, 
                     itemName, userName, departmentId, quantity, checkedOutBy)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    record_id,
                    data.get('item', ''),
    ...
    def do_PATCH(self):
        ...
        try:
            ...
            if resource == 'items':
                ...
                for field in ['firstName', 'lastName', 'cost_code', 'role']:
                    if field in data:
                        updates.append(f'{field} = ?')
                        values.append(data[field])
                ...
                for field in ['username', 'firstName', 'lastName', 'role']:
                    if field in data:
                        updates.append(f'{field} = ?')
                        values.append(data[field])
                ...
                for field in ['itemName', 'userName', 'departmentId', 'jobNum', 'checkedOutBy', 'notes']:
                    if field in data:
                        updates.append(f'{field} = ?')
                        values.append(data[field])
    ...
    def do_DELETE(self):
        ...
        try:
            ...
            if resource == 'items':
                ...
                cursor.execute('DELETE FROM auth_users WHERE id = ?', (resource_id,))
            elif resource == 'checkoutHistory':
