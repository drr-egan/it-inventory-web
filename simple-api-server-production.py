...
DATABASE = 'inventory-production.db'
...
class APIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.send_cors_error(409, 'Resource with this ID already exists')
        except Exception as e:
            print(f"Error in POST: {e}")
            self.send_cors_error(500, f'Server error: {str(e)}')
    ...
    def do_PATCH(self):
        ...
        if len(path_parts) != 2:
            self.send_cors_error(404, 'Resource not found')
            return
        ...
        try:
            ...
            self.send_cors_error(400, 'Invalid JSON')
            return
        ...
        try:
            ...
            if resource == 'items':
                ...
                if not updates:
                    self.send_cors_error(400, 'No valid fields to update')
                    return
                ...
                if cursor.rowcount == 0:
                    self.send_cors_error(404, 'Item not found')
                    return
                ...
                if cursor.rowcount == 0:
                    self.send_cors_error(404, 'Record not found')
                    return
                ...
                for field in ['firstName', 'lastName', 'cost_code', 'name']:
                    if field in data:
                        updates.append(f'{field} = ?')
                        values.append(data[field])
                ...
                if cursor.rowcount == 0:
                    self.send_cors_error(404, 'User not found')
                    return
